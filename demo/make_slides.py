...
for val, lab, col in metrics:
    ...
    x = Emu(x + Inches(3.1))
tf = tb(s, Inches(0.6), Inches(4.35), Inches(12.1), Inches(2.4))
txt(tf, "11,947 → 299 tokens · 1 frontier call avoided · task success preserved (tests pass)", 17, FG, bold=True, space=10)
txt(tf, "Real public repo (psf/requests): whole repo ≈147,794 tokens → ~1,025 for the right answer — 57×, −99.3%.", 17, TEAL, bold=True, space=10)
txt(tf, "Live now: Butterbase app app_b197i2548pk2 (real rows) · EverMind memory written · 27/27 tests.", 15, MUTED, space=10)
txt(tf, "Reproduce on stage:  autopilot index  ·  autopilot eval  ·  bash demo/run_demo.sh", 15, TEAL, bold=True)
